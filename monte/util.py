...
from numpy import ones, dot, newaxis, log, exp, size, sqrt, mean, std,\
                  floor, ceil, sum, vstack, hstack, where, isscalar, arange,\
 		  pi, concatenate, cos, sin
from pylab import find, zeros, reshape, imshow, show, rand, randn
...
def subsample(im, xout, yout):
    """ Subsample an image. """
    xin, yin = map(float, im.shape)
    indexesx = floor(arange(0,xin,xin/xout)).astype(int)
    indexesy = floor(arange(0,yin,yin/yout)).astype(int)
    return im[indexesx,:][:,indexesy]
...
def distmatrix_parallelThreads(x, y=None, numthreads=4):
    """ Compute distance matrix for the points in (columnwise) data-matrices 
        x and y. Parallel implementation using threads. 

        This is a parallel implementation that uses threads. When run on an
        SMP machine, it can lead to a significant speed-up compared to the 
        standard distmatrix-function. 
    """
    ...
    class distThread(threading.Thread):
        def run(self):
            while True:
                data_index = dataPool.get() 
                if data_index == None:
                    break
                dists[:, data_index[1][0]:data_index[1][1]] \
                                       = distmatrix(x, data_index[0])
    dists = zeros((x.shape[1], y.shape[1]), dtype=float)
    dataPool = Queue.Queue()
    chunksize = ceil(float(y.shape[1])/numthreads)
    for j in range(numthreads):
        dataPool.put((y[:, chunksize*j:chunksize*(j+1)], 
                                               (chunksize*j, chunksize*(j+1))))
    for j in range(numthreads):
        dataPool.put(None)
    threads = [distThread() for i in range(numthreads)] 
